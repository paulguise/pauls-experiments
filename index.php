...
if(isset($_REQUEST['email']) && $_REQUEST['email']<>'' && isset($_REQUEST['password']) && $_REQUEST['password']<>'' ){
  #check the pass
  $checkpass = grab_value("select password from users where email = '".$_REQUEST['email']."'");
  if (!empty($checkpass)){
    #they have an account, as their email is in the system. now compare the passwords
    if($checkpass == $_REQUEST['password']){
      #password matches, so login
      #put a cookie on their system so we know who the hell they are
      $theuser = grab_value("select user from users where email = '".$_REQUEST['email']."' and password = '".$checkpass."'");
      setcookie('dbrcid', $theuser, time()+60*60*24*30, '/');
      header('Location: roster.php');
    }else{
      #password does not match, so notify them
      $message = '<br /><br />The password is incorrect for that email.';
    }
  }else{
  ...
  $message = '<br /><br />You forget something there, chucklehead?';
  }else{
...
function login() {
    FB.login(function(response) {
        if (response.authResponse) {
          ...
          // cancelled
          //console.log('cancelled via fb window decline');
        }
    }, {scope: 'email'});
}
  ...
  function testAPI() {
    ...
    <div class="container">
        ...
        <form class="form-signin form-inline text-center" action="" method="post">
          ...
          <p class="">Enter your email and a password below in order to signin to your existing account or to signup for a new account. No confirmation emails, spams, security checks, or hoop jumping is required (yet).</p>
          <br>
          <input type="email" class="input-medium" placeholder="Email address" name="email">
          ...
          <?=$message?>
          <hr>
          <p> You may also login via Facebook if you wish.</p><br>
          <fb:login-button show-faces="false" size="xlarge" width="200" max-rows="1"></fb:login-button>
        </form>
