...
function filled_out_team_sheet($team){
	...
	$temp = '<table class="table table-hover table-striped span11">';
	while( $r = mysql_fetch_object($r_q) ) {
		//team name
		$temp .= '<tr>
					<td colspan=7></td>
					<td colspan=3>Dice: <a href="#" class="db-editable-field" id="teamdata_'.$r->team.'" data-type="number" data-placeholder="0"  data-name="dice" data-teamid="'.$r->team.'" data-pk="'.$r->team.'" data-url="push.php?a=teamdata" data-title="Dice" data-value="'.$r->dice.'">'.$r->dice.'</a> @ 6mc each</td>
					<td colspan=3>Cards: <a href="#" class="db-editable-field" id="teamdata_'.$r->team.'" data-type="number" data-placeholder="0"  data-name="cards" data-teamid="'.$r->team.'" data-pk="'.$r->team.'" data-url="push.php?a=teamdata" data-title="Cards" data-value="'.$r->cards.'">'.$r->cards.'</a> @ 10mc each</td>
				</tr>';
		...
		#print var_dump($skills_full).'<br><br><br>';
			...
			$temp .= "<tr><td>Player</td><td>Name</td><td>Role</td><td>Ex.</td><td>Rank</td><td>Move</td><td>Strength</td><td>Speed</td><td>Skill</td><td>Armor</td><td>Abilities</td><td><nobr>Special Rules</td><td>Cost</td></tr>";
		for($i=1; $i<=14; $i++){
			$current = $r->{'player_'.$i};
			if(!empty($current)){
				#team member present
		...
		//team motto
		$temp .= '<tr>
					<td colspan=3>Support Staff: <a id="coaches_'.$r->team.'" class="s2coaches" data-name="coaches" data-type="select2" data-pk="'.$r->team.'" data-value="'.$r->coaches.'" data-url="push.php?a=teamdata" data-title="coaches"></a></td>
					<td colspan=3>Motto: <a href="#" class="db-editable-field'.(empty($r->motto)?' editable-empty':'').'" data-type="textarea" data-rows="3" data-name="motto"  data-pk="'.$r->team.'" data-url="push.php?a=teamdata" data-title="Motto" data-placeholder="Motto motto man..." data-value="'.$r->motto.'">'.(empty($r->motto)?'Empty':$r->motto).'</a></td>
					<td colspan=3>Notes: <a href="#" class="db-editable-field'.(empty($r->notes)?' editable-empty':'').'" data-type="textarea" data-rows="3" data-name="notes"  data-pk="'.$r->team.'" data-url="push.php?a=teamdata" data-title="Notes" data-placeholder="Notes" data-value="'.$r->notes.'">'.(empty($r->notes)?'Empty':$r->notes).'</a></td>
					<td colspan=2>LP: <a href="#" class="db-editable-field" data-type="number" data-placeholder="0"  data-name="league_points"  data-pk="'.$r->team.'" data-url="push.php?a=teamdata" data-title="League Points" data-value="'.$r->league_points.'">'.$r->league_points.'</a></td>
					<td colspan=1>Cash: <a href="#" class="db-editable-field" data-type="number" data-placeholder="0"  data-name="cash" data-teamid="'.$r->team.'" data-pk="'.$r->team.'" data-url="push.php?a=teamdata" data-title="Extra Cash" data-value="'.$r->cash.'">'.$r->cash.'</a></td>
					</tr>
					<tr><td colspan=13 id="roster_cost_'.$r->team.'" style="font-weight:bold;">Total Roster Cost: '.get_roster_total($r->team).'mc</td>
				</tr>';
		#cost
	}
	$temp .= '</table>';
...
function player_input_row($team_type,$position,$line_num='',$teamid='',$use_tr=false){
	...
	if(isset($position) && $position<>''){
		...
		$temp .= '<td class="span1">'.$line_num.'</td>
			<td colspan="13" class="span12"><a href="#" id="playername_'.$line_num.'" class="add-player editable-empty" data-type="select" data-pk="'.$line_num.'" data-value="" data-source="get.php?a=getteamplayer&t='.$team_type.'" title="Select Player">Add Player</a></td>
			'.PHP_EOL;
		if($use_tr==true){
			$temp .= '</tr>'.PHP_EOL;
		}	
...
function player_input_row_filled($playerid,$abilities_array=''){
	...
	while( $r = mysql_fetch_object($r_q) ) {
		...
		$temp .= '<tr id="team'.$r->team.'_player'.$r->line_number.'" teamid="'.$r->team.'" playerline="'.$r->line_number.'" class="">
			<td class="span1">'.$r->line_number.'<a href="#deleteModal" data-toggle="modal" role="button" data-id="'.$r->team_member.'" data-playername="'.$r->name.'" id="killplayer_'.$r->team_member.'" class="killplayer icon-remove-sign pull-right"></a>
			</td>
			<td class="span3"><a href="#" class="db-editable-field" id="playername_'.$r->team_member.'" data-type="text" data-placement="right" data-name="name"  data-pk="'.$r->team_member.'" data-url="push.php?a=playerdata" data-title="Enter Player Name" data-value="'.$r->name.'">'.$r->name.'</a></td>
			...
			<td class="span5">
			<a id="playerabilities_'.$r->team_member.'" class="s2multiple" data-name="abilities" data-placement="left" data-type="select2" data-teamid="'.$r->team.'" data-pk="'.$r->team_member.'" data-value="'.$r->abilities.'" data-url="push.php?a=playerdata" data-title="Select Ability"></a></td>

			<td class="">'.display_special_rules($r->special_rule).'</td>
			<td class="span1 player-cost">'.($r->cost + ($rank>1 ? ($rank-1)*5 : 0 )).'</td>
			</tr>'.PHP_EOL;
	}
...
function new_player_input_row_blank($team,$position_id,$player_row_id=''){
	...
	while( $r = mysql_fetch_object($r_q) ) {
		#experience defines rank and cost. for each rank, add 5 to the cost.
		$rank = player_rank($r->experience);
		$temp .= '<td class="span1">'.$r->line_number.' <a href="#deleteModal" data-toggle="modal" role="button" data-id="'.$r->team_member.'" data-playername="'.$r->name.'" id="killplayer_'.$r->team_member.'" class="killplayer icon-remove-sign pull-right"></a></td>
			<td class="span3"><a href="#" class="db-editable-field" id="playername_'.$r->team_member.'" data-placement="right" data-name="name" data-type="text" data-pk="'.$r->team_member.'" data-url="push.php?a=playerdata" data-title="Enter Player Name" data-value="Player Name '.$r->line_number.'">'.$r->name.'</a></td>
			<td class="span1">'.player_pos($r->position).'</td>
			<td class=""><a href="#" class="db-editable-field" id="pexperience_'.$r->team_member.'" data-placement="right" data-type="number" data-placeholder="0" data-name="experience" data-teamid="'.$r->team.'" data-pk="'.$r->team_member.'" data-url="push.php?a=playerdata" data-title="Experience Points" data-value="'.$r->experience.'">'.$r->experience.'</a></td>
			<td class="player-rank">'.$rank.'</td>
			<td class="">'.$r->move.'</td>
			<td class="player-strength">'.$r->strength.'</td>
			<td class="player-speed">'.$r->speed.'</td>
			...
			<td class="player-armor">'.$r->armor.'</td>
			<td class="span5">
			<a class="s2multiple"  id="playerabilities_'.$r->team_member.'" data-name="abilities" data-placement="left" data-type="select2" data-teamid="'.$r->team.'" data-pk="'.$r->team_member.'" data-value="'.$r->abilities.'" data-url="push.php?a=playerdata" data-title="Select Ability"></a></td>
			<td class="">'.display_special_rules($r->special_rule).'</td>
			<td class="span1 player-cost">'.($r->cost + ($rank>1 ? ($rank-1)*5 : 0 )).'</td>
			'.PHP_EOL;
	}
...
function new_team($team_type,$team_name='Uncreative Team Name'){
	...
	while( $r = mysql_fetch_object($r_q) ) {
		...
		$i_q ="insert into teams (team_name,team_type,user,dice,cards,cash,created,active) values ('".$team_name."',".$team_type.",".$_COOKIE['dbrcid'].",".$r->start_dice.",".$r->start_cards.",0,'".date('Y-m-d')."',1)";
...
function get_roster_total($team){
	...
	while( $r = mysql_fetch_object($r_q) ) {
		...
		$q2 = "select * from team_members where team=".$team;
